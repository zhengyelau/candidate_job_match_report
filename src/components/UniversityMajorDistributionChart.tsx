...
interface UniversityMajorDistributionChartProps {
  results: MatchResult[];
  onCandidateClick: (candidateId: number) => void;
  selectedCandidateIds: Set<number>;
}

export function UniversityMajorDistributionChart({ results, onCandidateClick, selectedCandidateIds }: UniversityMajorDistributionChartProps) {
  const calculateDistribution = () => {
    const majorTypes = new Map<string, MatchResult[]>();

    results.forEach(result => {
      const major = result.candidate.past_current_university_major || 'Not Specified';
      const normalizedMajor = major.trim() || 'Not Specified';

      if (!majorTypes.has(normalizedMajor)) {
        majorTypes.set(normalizedMajor, []);
      }
      majorTypes.get(normalizedMajor)?.push(result);
    });

    return Array.from(majorTypes.entries())
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">University Major Distribution</h2>
      ...
      <div className="bg-slate-50 rounded-lg p-8 border border-slate-200">
            ...
            <div className="relative h-full flex items-end justify-around px-4 gap-2">
              {distribution.map((bucket, index) => (
                <div key={index} className="flex-1 flex flex-col items-center justify-end h-full relative">
                  <div
                    className="w-full bg-cyan-200 rounded-t transition-all duration-300 relative"
                    style={{
                        ...
                        return (
                          <div
                            ...
                            className={`w-3 h-3 rounded-full shadow-sm cursor-pointer transition-all hover:scale-125 ${
                              isSelected ? 'bg-yellow-400 ring-2 ring-yellow-300' : 'bg-cyan-600 hover:bg-cyan-700'
                            }`}
                            title={`${candidate.candidate.first_name} ${candidate.candidate.last_name} - Major: ${candidate.candidate.past_current_university_major || 'Not Specified'}`}
                          />
                        );
                      })}
                    </div>
        ...
        <div className="text-center mt-4">
          <div className="text-base font-semibold text-slate-800">University Major</div>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600">
              This histogram shows the distribution of candidates by their university major. Click on any cyan dot to view that candidate's details.
            </p>
          </div>
        </div>
      </div>
