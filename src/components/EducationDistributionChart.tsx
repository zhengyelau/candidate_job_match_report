...
export function EducationDistributionChart({ results, onCandidateClick, selectedCandidateIds }: EducationDistributionChartProps) {
  const calculateDistribution = () => {
    const educationTypes = new Map<string, MatchResult[]>();

    results.forEach(result => {
      const education = result.candidate.past_current_education_subject || 'Not Specified';
      const normalizedEducation = education.trim() || 'Not Specified';

      if (!educationTypes.has(normalizedEducation)) {
        educationTypes.set(normalizedEducation, []);
      }
      educationTypes.get(normalizedEducation)?.push(result);
    });

    return Array.from(educationTypes.entries())
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Education Distribution</h2>
      ...
      <div className="bg-slate-50 rounded-lg p-8 border border-slate-200">
            ...
            <div className="relative h-full flex items-end justify-around px-4 gap-2">
              {distribution.map((bucket, index) => (
                <div key={index} className="flex-1 flex flex-col items-center justify-end h-full relative">
                  <div
                    className="w-full bg-sky-200 rounded-t transition-all duration-300 relative"
                    style={{
                        ...
                        return (
                          <div
                            ...
                            className={`w-3 h-3 rounded-full shadow-sm cursor-pointer transition-all hover:scale-125 ${
                              isSelected ? 'bg-yellow-400 ring-2 ring-yellow-300' : 'bg-sky-600 hover:bg-sky-700'
                            }`}
                            title={`${candidate.candidate.first_name} ${candidate.candidate.last_name} - Education: ${candidate.candidate.past_current_education_subject || 'Not Specified'}`}
                          />
                        );
                      })}
                    </div>
        ...
        <div className="text-center mt-4">
          <div className="text-base font-semibold text-slate-800">Education Subject</div>
        </div>
      </div>

      <div className="mt-4 p-4 bg-sky-50 rounded-lg border border-sky-200">
        <div className="flex items-start gap-3">
          <div className="w-3 h-3 bg-sky-600 rounded-full mt-1 flex-shrink-0"></div>
          <div>
            <p className="text-sm font-medium text-slate-800 mb-1">
              Instructions:
            </p>
            <p className="text-xs text-slate-600">
              This histogram shows the distribution of candidates by their education subject. Click on any sky blue dot to view that candidate's details.
            </p>
          </div>
        </div>
      </div>
